...
def infer(fea, training=True):
    ...
    with tf.variable_scope("dnn"):
        ...
        logits = tf.layers.dense(states, 24, activation=leaky_relu, kernel_initializer=init)
        if training:
            logits = tf.nn.dropout(logits, 0.8)
        init = tf.truncated_normal_initializer(stddev=1.0 / math.sqrt(12.0))
        logits = tf.layers.dense(logits, 24, activation=leaky_relu, kernel_initializer=init)
        if training:
            logits = tf.nn.dropout(logits, 0.8)
