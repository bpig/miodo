...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.loss_type == 'square_loss':
                ...
                if self.lamda_bilinear > 0:
                    self.loss = tf.losses.log_loss(self.out, self.train_labels) + \
                                tf.contrib.layers.l2_regularizer(
                                    self.lamda_bilinear)(self.weights['feature_embeddings'])  # regulizer
                else:
                    # self.loss = tf.nn.sigmoid_cross_entropy_with_logits(
                    #     labels=self.train_labels, logits=self.out)
                    self.loss = tf.losses.log_loss(self.out, self.train_labels)
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0:  # with pretrain
            ...
            # features_M * K
            all_weights['feature_embeddings'] = tf.get_variable(
                shape=[self.features_M, self.hidden_factor],
                initializer=tf.truncated_normal_initializer(stddev=0.1),
                name='feature_embeddings')
            # features_M * 1
            all_weights['feature_bias'] = tf.get_variable(
                shape=[self.features_M, 1],
        ...
        if num_layer > 0:
            ...
            all_weights['bias_0'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(1, self.layers[0])), dtype=np.float32)  # 1 * layers[0]
            ...
            all_weights['prediction'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(self.layers[-1], 1)), dtype=np.float32)  # layers[-1] * 1
        else:
            all_weights['prediction'] = tf.Variable(
    ...
    def evaluate(self, data, name):  # evaluate the results for an input set
        ...
        if self.loss_type == 'square_loss':
            ...
            logloss = log_loss(y_true, y_pred, eps=1e-7)  # I haven't checked the log_loss
            return logloss
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print(
            "Neural FM: dataset=%s, hidden_factor=%d, dropout_keep=%s, "
            "layers=%s, loss_type=%s, pretrain=%d, #epoch=%d, batch=%d, "
            "lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s, early_stop=%d"
